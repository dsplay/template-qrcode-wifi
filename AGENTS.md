...
The DSPLAY **Wi-Fi QR Code** template — a [React](https://reactjs.org/) app built with [Vite](https://vitejs.dev/), showing a full-screen QR code that connects a phone to a Wi-Fi network when scanned. Requires Node.js 22.22.2+, 24.15.0+, or 26+ (see `.nvmrc`). See README.md for the template's variables.
...
src/
  ...
  components/
    app/                      <-- top-level component (loader, fonts, i18n, background image)
    main/                     <-- lays out the Wi-Fi symbol + QR code
    wifi-qrcode/              <-- builds the WIFI: connection string and renders the QR code
...
- **Every static, developer-authored piece of UI text must go through `react-i18next`'s `t()`** — never a hardcoded string in JSX. This template has exactly one such string (the Wi-Fi symbol's `alt` text).
